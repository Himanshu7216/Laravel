...
<head>
    ...
    <style>
        ...
        .ck-tabs__panels-container_right {
            /* display: none; */
        }
    </style>
...
<body>
    ...
    <div class="container mt-5">
        <div class="card shadow">
            ...
            <div class="card-body">
                <form method="POST" action="/send_email" id="emailForm" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="dropdown w-100" id="emailDropdown">
                            <button id="dropdownBtn" class="btn btn-outline-secondary dropdown-toggle w-100 text-start"
                                type="button" data-bs-toggle="dropdown">
                                Select Recipients
                            </button>

                            <ul class="dropdown-menu w-100 p-3" style="max-height: 200px; overflow-y: auto;">
                                @if ($users)
                                    @foreach ($users as $user)
                                        <li>
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="emails[]"
                                                    id="email_{{ $user->id }}" value="{{ $user->email }}">
                                                <label class="form-check-label"
                                                    for="email_{{ $user->id }}">{{ $user->email }}</label>
                                            </div>
                                        </li>
                                    @endforeach
                                @else
                        ...
                        <div class="text-danger error-emails"></div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="editor-container editor-container_classic-editor editor-container_include-annotations editor-container_include-word-count editor-container_include-fullscreen"
                            id="editor-container">
    ...
    <script>
        ...
        const LICENSE_KEY = 'eyJhbGciOiJFUzI1NiJ9..TprOlaEjZjqWqLB8vmmVjQCUFRF2MFtARURQt0wiaK9FCktGDZY_YfbIzBU_tfdKBGP6NGf0SPQHfWkhLVX49A';
        ...
        const CLOUD_SERVICES_TOKEN_URL = 'https://2cqhvijs8bct.cke-cs.com/token/dev/938d42694c33d000d9b1b24f2969490b993ce84244d44de58fc1e10c282a?limit=10';
        const CLOUD_SERVICES_WEBSOCKET_URL = 'wss://2cqhvijs8bct.cke-cs.com/ws';
    ...
    <script>
        $(document).ready(function () {
            ...
            //  Subject Validation
            $("input[name='subject']").on("input blur", function () {

                let subject = $(this).val().trim();

                if (subject === "") {
                    $(".error-subject").text("Subject is required");
                } else if (subject.length < 3) {
                    $(".error-subject").text("Minimum 3 characters required");
                } else {
                    $(".error-subject").text("");
                }
            });
            ...
            $("input[name='emails[]']").on("change", function () {

                if ($("input[name='emails[]']:checked").length === 0) {
                    $(".error-emails").text("Please select at least one email");
                } else {
                    $(".error-emails").text("");
                }
            });
            //  Show selected email count in dropdown button
            $("input[name='emails[]']").on("change", function () {

                let count = $("input[name='emails[]']:checked").length;

                if (count > 0) {
                    $("#dropdownBtn").text(count + " recipient(s) selected");
                    $(".error-emails").text(""); // remove error if any
                } else {
                    $("#dropdownBtn").text("Select Recipients");
                }
            ...
            $("#emailForm").submit(function (e) {
                ...
                if (isEditorLoaded) {
                    message = CKEDITOR.instances.editor.getData().trim();
                } else {
                    message = $("#editor").val().trim();
                }

                let plainText = message.replace(/<[^>]*>/g, '').trim();
